  ...
  switch (sortBy) {
    ...
    case 'merasteem':
      return steemAPI.sendAsync(`get_discussions_by_blog`, [query]).then(blog => {
        // return only posts with #merasteem as first tag
        ...
        // more #merasteem posts in the next result set.
        // Ugly Solution: Return here not only the #merasteem posts but also the number of all posts that were returned by the API.
        // The hasMore check could then refer to that instead of the number of #merasteem posts.
        return blog.filter(post => {
          const meta = jsonParse(post.json_metadata);
          return meta.tags.indexOf('merasteem') === 0;
        });
      });
    default:
